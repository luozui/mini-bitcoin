...
func DumpUTXO(filename string, utxoset *utxo.UTXO) error {
	...
	for k, v := range *utxoset {
		var bufferk, bufferv bytes.Buffer
		encoderk, encoderv := gob.NewEncoder(&bufferk), gob.NewEncoder(&bufferv)
		errk := encoderk.Encode(&k) //编码
		errv := encoderv.Encode(&v)
		if errk != nil {
			log.Panic(errk)
		}
		if errv != nil {
		...
		e := db.Put(bufferk.Bytes(), bufferv.Bytes(), nil)
		if e != nil {
			return e
		}
	}
...
func encoderk(val interface{}) []byte {
	var buffer bytes.Buffer
	encoder := gob.NewEncoder(&buffer)
	err := encoder.Encode(&val) //编码
	if err != nil {
		log.Panic(err)
	}
	return buffer.Bytes()
